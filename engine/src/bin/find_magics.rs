use std::fs::File;
use std::io::Write;
use engine::magic_constants::{BISHOP_RELEVANT_BITS, ROOK_RELEVANT_BITS};
use engine::magics::{
...
fn main() {
    write_to_file("src/magic_constants.rs");
}
...
fn write_to_file(path: &str) {
    ...
    writeln!(file, "pub const BISHOP_MAGICS_ARRAY: [u64; 64] = [").unwrap();
    for square in 0..64 {
